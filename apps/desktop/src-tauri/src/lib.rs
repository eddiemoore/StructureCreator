...
#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_list_plugins(state: State<Mutex<AppState>>) -> Result<Vec<database::Plugin>, String> {
    let state = state.lock().map_err(|e| e.to_string())?;
...
#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_get_plugin(state: State<Mutex<AppState>>, id: String) -> Result<Option<database::Plugin>, String> {
    let state = state.lock().map_err(|e| e.to_string())?;
...
#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_install_plugin(state: State<Mutex<AppState>>, source_path: String) -> Result<database::Plugin, String> {
    use std::path::PathBuf;

    let source = PathBuf::from(&source_path);

    // Install plugin from source path
    let (dest_path, manifest) = plugins::install_plugin_from_path(&source)
        .map_err(|e| e.to_string())?;
...
#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_uninstall_plugin(state: State<Mutex<AppState>>, id: String) -> Result<bool, String> {
    use std::path::PathBuf;

    let state = state.lock().map_err(|e| e.to_string())?;

    // Get plugin to find its path
    let plugin = state.db.get_plugin(&id)
        .map_err(|e| e.to_string())?
        .ok_or_else(|| "Plugin not found".to_string())?;

    // Remove from filesystem
    let plugin_path = PathBuf::from(&plugin.path);
    plugins::uninstall_plugin(&plugin_path)
        .map_err(|e| e.to_string())?;
...
#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_enable_plugin(state: State<Mutex<AppState>>, id: String) -> Result<Option<database::Plugin>, String> {
    let state = state.lock().map_err(|e| e.to_string())?;
...
#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_disable_plugin(state: State<Mutex<AppState>>, id: String) -> Result<Option<database::Plugin>, String> {
    let state = state.lock().map_err(|e| e.to_string())?;
...
#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_get_plugin_settings(state: State<Mutex<AppState>>, id: String) -> Result<Option<serde_json::Value>, String> {
    let state = state.lock().map_err(|e| e.to_string())?;
    state.db.get_plugin_settings(&id).map_err(|e| e.to_string())
}

#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_save_plugin_settings(
    state: State<Mutex<AppState>>,
    id: String,
    settings: serde_json::Value,
) -> Result<Option<database::Plugin>, String> {
    let state = state.lock().map_err(|e| e.to_string())?;
    state.db.save_plugin_settings(&id, settings).map_err(|e| e.to_string())
}

#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_scan_plugins() -> Result<Vec<plugins::PluginManifest>, String> {
    let plugins = plugins::scan_plugins_directory()
        .map_err(|e| e.to_string())?;
    Ok(plugins.into_iter().map(|(_, manifest)| manifest).collect())
}

#[cfg(feature = "tauri-app")]
#[tauri::command]
fn cmd_sync_plugins(state: State<Mutex<AppState>>) -> Result<Vec<database::Plugin>, String> {
    use std::collections::HashSet;

    // Scan the filesystem for plugins
    let scanned = plugins::scan_plugins_directory()
        .map_err(|e| e.to_string())?;

    let state = state.lock().map_err(|e| e.to_string())?;

    // Get current database plugins
    let db_plugins = state.db.list_plugins()
        .map_err(|e| e.to_string())?;
